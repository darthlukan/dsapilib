...
MISSING_API_KEY_MSG = '''
Unable to instantiate DarkSkyAPI without key attribute. Are you missing
an api_key arg?
'''
...
MOCK_DATA = {
  ...
  "daily": {
      "summary": "Mixed precipitation throughout the week, with temperatures falling to 39F on Saturday.",
      "icon": "rain",
      "data": [{
          "time": 1509944400,
...
class DarkSkyAPI(object):
    def __init__(self, api_key):
        print('Have key: {0}, len: {1}'.format(api_key, len(api_key)))

        if len(api_key) == 32:
            self.key = api_key
        else:
            raise AttributeError(MISSING_API_KEY_MSG)

    def get_weather(self, lat, lon):
        action = 'forecast'
        url = '{0}/{1}/{2}/{3},{4}'.format(
                BASE_URL, action, self.key, lat, lon)
        # Prod
